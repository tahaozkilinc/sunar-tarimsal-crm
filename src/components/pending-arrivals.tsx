...
export function PendingArrivals({ role }: { role: Role }) {
  ...
  const load = async (silent = false) => {
    ...
    // gemileri güvenli kolonlu external_contracts görünümünden alırlar.
  ...
  const pending = visible.filter((c) => c.status !== "arrived");
  const arrived = visible.filter((c) => c.status === "arrived");
